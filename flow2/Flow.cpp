          ...
          int n,float *e,float *f,int k,float *g,float *b,float *dd, vector<int> PQ)
{
	...
	for(i=1;i<=l;i++)
	{
		...
		if((find(PQ.begin(),PQ.end(),i))!=PQ.end())
		{	/* 计算PQ结点中的deltaQi　*/
...
void jmcc(int m,int n,int n0,float *e,float *f,float *g,float *b,float *jm,int k, vector<int> PQ, vector<int> PV)
{
	FILE *file4 = NULL;
	FILE *fp;
	int i,j,i1,io,ns;
	...
	for(i=1;i<=ns;i++)
	{
		...
		for(i1=1;i1<=n;i1++)
		{
		    ...
		    if((find(PQ.begin(),PQ.end(),i))!=PQ.end()) /* i是PQ结点 */
			{
		...
		if((find(PV.begin(),PV.end(),i))!=PV.end()) /* PV结点 */
		{
			/* 计算式(4-90)中的Rii */
...
{
	FILE *file4 = NULL;/**file6;*/
	FILE *fp;
	float t1,t2,cm,x,y,z,x1,x2,y1,y2;
	int i,i1,j,ns,pos1,pos2,km,st,en;
	ns=n-1;
	if(file4==NULL)
	{
		fp=stdout;
	...
	if(k1==1)
	{
		...
		{
			angle[i]*=(float)Pi;
			e[i]=v[i]*(float)cos(angle[i]);
			f[i]=v[i]*(float)sin(angle[i]);
		}
	}
	t1=0.0;t2=0.0;
	for(i=1;i<=n;i++)
	...
	for(i=0;i<n;i++)
	{
		v[i]=(float)sqrt(e[i]*e[i]+f[i]*f[i]);
		x=e[i];
		y=f[i];
		z=y/x;
		angle[i]=(float)atan(z);
		angle[i]/=(float)Pi;
		//fprintf(fp,"\n%3d%13.5e%15.5f%15.5e%15.5e",i+1,v[i],angle[i],p[i],q[i]);
		fprintf(fp,"\n%3d\t%f\t%f\t%f\t%f",i+1,v[i],angle[i],p[i],q[i]);
	}
	fprintf(fp,"\n LINE FLOW ");
	for(i=1;i<=l;i++)
	{
		...
		fprintf(fp, "\n %d\t%d\t%d\t%f\t%f", i, s1[pos1], e1[pos1], p3[pos1], q3[pos1]);
	}
}	