...
int main()
{
	...
	errno_t err;					// 错误代码
	FILE *fid;
	err= fopen_s(&fid,"data.txt","r");				// 输入数据文件data.txt
	...
	std::ofstream fout("iteration.txt");

	ybus(N,L,M,G,B,G1,B1,C1,C,C0,K,S1,E1);			// 求导纳矩阵
	dpqc(P,Q,P0,Q0,V,V0,M,N,E,F,K,G,B,dd);			// 求deltaP, deltaQ, deltaV2
	int cnt = 0;									// 迭代计数器
	while ((*dd) > EPSILON)
	{
		cnt++;
		jmcc(M,N,N0,E,F,G,B,JM,K);					// 求雅可比矩阵
		...
		dpqc(P,Q,P0,Q0,V,V0,M,N,E,F,K,G,B,dd);		// 求deltaP, deltaQ
		printf("\n迭代次数：%d, 误差：%f\n",cnt,(*dd));
		fout << cnt << "\t" << (*dd) << std::endl;
		//system("pause");
	}
	printf("总的迭代次数：%d",cnt);
	plsc(N,L,M,G,B,E,F,E1,S1,G1,B1,C1,C,C0,P1,Q1,P2,Q2,P3,Q3,P,Q,V,ANGLE,K1);
